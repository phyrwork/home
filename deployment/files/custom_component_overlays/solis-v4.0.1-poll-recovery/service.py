...
# Bound one complete portal update.  Individual HTTP requests have their own
# timeout, but an update can contain login, throttling delays, and several
# control reads.  Without a total bound a stuck coroutine prevents the next
# scheduled update from ever being registered.
...
class InverterService:
    ...
    async def async_update(self, *args) -> None:
        ...
        if args:
            # The one-shot scheduled callback has fired and removed itself.
            self._unsub_update = None
        else:
            # A manual update replaces any pending scheduled update.
        ...
        try:
            async with async_timeout.timeout(UPDATE_TIMEOUT_SECONDS):
                # Login using username and password, but only every
                # HRS_BETWEEN_LOGIN hours.
                if await self._login():
                    inverters = self._api.inverters
                    if inverters is None:
                        return
            ...
            _LOGGER.warning("Timed out updating Solis portal after %s seconds", UPDATE_TIMEOUT_SECONDS)
            # Ensure a timed-out session is not reused on the next attempt.
            await self._logout()
        finally:
            # Always register the next update, including when an API call hangs
            # or when there are no inverters available yet.  A service being
            # unloaded must not create a new timer as its update completes.
            if not self._shutdown:
                self.schedule_update(update)

    def _cancel_pending_update(self) -> None:
        """Cancel the currently scheduled update, if any."""
        cancel = self._unsub_update
        self._unsub_update = None
    ...
    def schedule_update(self, td: timedelta) -> None:
        ...
        self._unsub_update = async_track_point_in_utc_time(self._hass, self.async_update, nxt)
