...
def solis_result(soc: str = "50") -> SolisStateReadResult:
    telemetry = SolisTelemetry(
        Decimal(soc), Decimal("0"), NOW, NOW, NOW, NOW
    )
...
async def test_reads_planner_input(hass: HomeAssistant) -> None:
    ...
    with patch.object(
        ...
        result = await inputs.async_read_input(
            hass, config(), now=NOW, solis_result=solis_result()
        )
...
async def test_requires_real_solis_soc(hass: HomeAssistant) -> None:
    set_valid_states(hass)

    with pytest.raises(ValueError, match="real Solis SOC is unavailable"):
        await inputs.async_read_input(hass, config(), now=NOW)


async def test_real_solis_soc_drives_planner_state_without_soc_helper(
    hass: HomeAssistant,
) -> None:
    set_valid_states(hass)

    with patch.object(inputs, "_async_get_solar_forecast", return_value=solar_forecast()):
        result = await inputs.async_read_input(
            hass, config(), now=NOW, solis_result=solis_result("37")
        )

    assert result.battery_state.energy_kwh == Decimal("11.896832")
